...
```
- It can set a cookie domain to ITSELF or PARENTS `x.y.z.com`, `y.z.com`, `z.com`. But not `com`, which is a public suffix
...
```
- PATH表示cookie所在的目錄，`asp.net`默認為`/`為root 

FOR EXAMPLE  
同一個服務器(Server)上有以下Directories  
```diff
+ /test/
- /test/cd/
- /test/dd/
...
/**
  ...
  * And MyServlet1 fetchs this request
  */
import java.io.*;
...
HttpServlet handle these things while fetching a request from the client
